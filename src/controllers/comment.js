...
var controller = {
    ...
    saveComment: function(req, res){
        let newcomentario = new Comment();
        let params = req.body;
        newcomentario.nombre = params.nombre;
        newcomentario.comentario = params.comentario;
        newcomentario.save((err, commentStored) => {
            if(err) return res.status(500).send({meesage: "hubo un error guardando macho"});

            if(!commentStored) return res.status(404).send({message: "no se pudo guardar weon"});

            return res.status(200).send({product: commentStored, message: "guardado correctamente"});
        });
    },
